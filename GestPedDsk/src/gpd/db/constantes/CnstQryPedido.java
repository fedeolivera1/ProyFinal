...
public interface CnstQryPedido {
	...
	public static final String QRY_SELECT_PEDIDO_XTRANSAC = "SELECT p.id_persona, p.fecha_hora, p.estado, p.fecha_prog, p.hora_prog, p.origen, "
												+ "p.sub_total, p.iva, p.total, p.nom_usu, p.nro_transac, p.sinc, p.ult_act "
												+ "FROM pedido p "
												+ "WHERE p.nro_transac = ? ";
	...
	public static final String QRY_ACT_ESTADO_PEDIDO = "UPDATE pedido "
												+ "SET estado = ?, sinc = ?, ult_act = ? "
												+ "WHERE id_persona = ? "
												+ "AND fecha_hora = ?";
